...
def test_create_order_reduces_stock(db_phase1, seeded):
    ...
    db_phase1.create_order(customer_id=1, items=[{"product_id": 1, "quantity": 25}])

    # Next order for even 1 unit should fail — stock is exhausted
    with pytest.raises(ValueError, match="Insufficient stock"):
        db_phase1.create_order(customer_id=1, items=[{"product_id": 1, "quantity": 1}])
...
def test_revenue_by_category(db_phase1, seeded):
    ...
    db_phase1.create_order(customer_id=1, items=[{"product_id": 1, "quantity": 1}])
    db_phase1.create_order(customer_id=2, items=[{"product_id": 2, "quantity": 1}])
